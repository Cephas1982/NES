...
void C_CPU::GetNextCode()
{
	...
	if(m_flagD)
		pStatus |= fD;//3
	if(m_flagB){
		pStatus |= fB;//5 This can only be set by a BRK instruction or interrupt *** DOESN'T EXIST ***		
	}
	if(m_emptyFlag)
		pStatus |= fE;//5 ---- also not used
	if(m_flagV)
		pStatus |= fV;//6
	if(m_flagN)
		pStatus |= fN;//7
	...
	//mult * 3 b/c there's no PPU emu ( YET! )
	int cycles = (m_cycleCount * 3)% 341;
	if(cycles > 341){
		cycles -= 341;
		m_scanlineCount++;
		m_cycleCount =  cycles-1; //rigged up at the moment b/c I increment below
	}
